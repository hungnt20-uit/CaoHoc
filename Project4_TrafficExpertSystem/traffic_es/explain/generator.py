...
def render_explanation(
    ...
    ket_qua: KetQua,
    *,
    missing_vehicle: bool = False,
    missing_behavior: bool = False,
    missing_level: bool = False,
) -> str:
    if not ket_qua.chi_tiet:
        if missing_level:
            return (
                "Đã nhận chủ đề vi phạm nhưng chưa đủ mức cụ thể — "
                "vui lòng chọn khung bên dưới để tính mức phạt."
            )
        if missing_behavior:
            return (
                "Chưa xác định được hành vi vi phạm cụ thể — "
                "vui lòng chọn lỗi bên dưới hoặc mô tả rõ hơn."
            )
        if missing_vehicle:
            return (
                "Đã nhận hành vi liên quan nhưng chưa xác định loại phương tiện — "
                "vui lòng bổ sung để tính mức phạt."
            )
